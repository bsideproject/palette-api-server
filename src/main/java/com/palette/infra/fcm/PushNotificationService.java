...
public class PushNotificationService {
    ...
    public void historyCreated(History history) throws FirebaseMessagingException {
        ...
        String noteBody = Duration.between(history.getStartDate(), history.getEndDate()).toDays() + "일 후 상대방에게 보여주고 싶은 나의 하루를 기록해보세요.";
        HashMap<String, String> noteData = new HashMap<>();
    ...
    public void pageCreated(Page page) throws FirebaseMessagingException {
        ...
        List<User> users = userRepository.findUsers(history);
        User author = users.stream().filter(user -> user.getId().equals(page.getUserId())).findFirst().orElseThrow(UserNotFoundException::new);
        users.remove(author);
